...
import { TrendingUp, TrendingDown, Clock, Check, Zap, Target, Users } from 'lucide-react';

const ResultsSection = () => {
  const outcomes = [
    { text: '+127% PDP conversion', icon: TrendingUp, color: 'text-accent' },
    { text: '-65% SEM costs', icon: TrendingDown, color: 'text-primary' },
    { text: '80% faster time to shelf', icon: Clock, color: 'text-secondary' },
    { text: 'Stronger SEO rankings', icon: Target, color: 'text-accent' },
    { text: 'Content that differentiates — not commoditises', icon: Zap, color: 'text-primary' },
    { text: 'Content teams scale without scaling headcount', icon: Users, color: 'text-secondary' }
  ];
          ...
          {outcomes.map((outcome, index) => (
            <div key={index} className="flex items-start gap-4 p-6 rounded-3xl bg-card border border-border hover:shadow-soft transition-all duration-300 group hover:border-primary/20" style={{ animationDelay: `${index * 0.1}s` }}>
              ...
              <div className="flex-1">
                <p className="text-base font-medium leading-relaxed text-foreground group-hover:text-primary transition-colors">{outcome.text}</p>
              </div>
            </div>
          ))}
        </div>
